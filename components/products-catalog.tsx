...
export default function ProductsCatalog({
	...
	return (
		<div className="mx-auto max-w-6xl px-4 py-12">
			...
			<p className="mt-3 max-w-3xl text-sm text-slate-600 md:text-base">{description}</p>
					...
					return (
						<button
							...
							className={`rounded-full px-4 py-2 text-sm font-semibold transition ${
								isActive
									? "bg-gradient-to-r from-sky-600 to-cyan-500 text-white shadow-md shadow-sky-400/35"
				...
				{filteredItems.map((product) => (
					...
					>
						<div className="relative">
							...
							<p className="absolute bottom-3 left-3 inline-flex rounded-full bg-white/90 px-3 py-1 text-xs font-semibold text-slate-700">{product.categoryLabel}</p>
						</div>
						<div className="p-5 md:p-6">
							...
							<button
								...
								className="mt-4 inline-flex rounded-full border border-sky-200 bg-sky-50 px-4 py-2 text-sm font-semibold text-sky-700 transition hover:border-sky-300 hover:bg-sky-100"
							>
								{viewDetailLabel}
							</button>
			...
			{selectedProduct ? (
				<div className="fixed inset-0 z-50 flex items-end bg-slate-950/55 p-0 md:items-center md:justify-center md:p-6" onClick={() => setSelectedProductId(null)}>
					<div
						className="max-h-[88vh] w-full overflow-auto rounded-t-3xl bg-white p-5 shadow-2xl md:max-w-2xl md:rounded-2xl md:p-6"
						onClick={(event) => event.stopPropagation()}
					>
						<div className="mb-4 flex items-start justify-between gap-3">
							<div>
								<p className="inline-flex rounded-full bg-sky-50 px-3 py-1 text-xs font-semibold text-sky-700">{selectedProduct.categoryLabel}</p>
